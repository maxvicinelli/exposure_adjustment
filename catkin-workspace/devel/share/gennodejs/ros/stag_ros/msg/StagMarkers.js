...
class StagMarkers {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.markers[i] = StagMarker.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new StagMarkers(null);
    ...
    if (msg.markers !== undefined) {
      ...
      for (let i = 0; i < resolved.markers.length; ++i) {
        resolved.markers[i] = StagMarker.Resolve(msg.markers[i]);
      }
    }
    else {
      resolved.markers = []
